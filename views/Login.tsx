...
const Login: React.FC<Props> = ({ onLogin }) => {
  ...
  const handleGoogleLogin = () => {
    ...
    setTimeout(() => {
      const email = 'demo.pareja@gmail.com';
      let user = mockDb.findUserByEmail(email);
      
      if (!user) {
        // Create a default user if they don't exist yet (simulating first time Google login)
        user = {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[80vh] px-4 transition-theme">
      <div className="w-full max-w-sm bg-white dark:bg-gray-900 p-8 rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-800 text-center">
        ...
        <h1 className="text-3xl font-black text-gray-800 dark:text-gray-100 mb-2">Meta Pareja</h1>
        <p className="text-gray-500 dark:text-gray-400 mb-10 text-sm">Tus sueños financieros compartidos, sin complicaciones.</p>
        ...
        <p className="mt-10 text-xs text-gray-400 dark:text-gray-500">
          No necesitas contraseña. Al continuar, aceptas nuestros <a href="#" className="underline">Términos de Servicio</a>.
        </p>
      </div>
      
      <div className="mt-8 text-center text-gray-400 dark:text-gray-500 text-sm">
        Desarrollado con ❤️ para parejas financieras
      </div>
    </div>
  );
};
